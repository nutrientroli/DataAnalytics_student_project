...
```{r, eval=TRUE, echo=TRUE}
  ...
  plot(dauMauuAglomerated,
       main = "Stickiness en base al dia",
       xlab = "Dia de la muestra",
       ylab = "Ratio DAU/MAUU",
       type = "l")
...
```{r eval=TRUE, echo=TRUE, warning=FALSE}
  ...
  averageDauMaus
  plot(averageDauMaus,
       main = "Media de DAU/MAUU por cada mes",
       xlab = "Mes",
       ylab = "Valor medio del mes",
       color = "blue",
       type = "l"
       )
  
  # Interpretacion B (media de los dau de un mes partido el mauu de este)
...
```{r, eval=TRUE, echo=TRUE}
  ...
  hist(userSessionsAverage$TimeSession, 
          main = "Tiempo de juego", 
          col = rainbow(length(table(userSessionsAverage$TimeSession))),
          ylab = "Usuarios (número de usuarios)",
          xlab = "Tiempo de juego",
          xlim = c(min(userSessionsAverage$TimeSession)-100,max(userSessionsAverage$TimeSession)+100),
          ylim = c(0,1000)
          )
  
```

Se observan dos rangos predominantes, de 180 a 200 unidades temporales (parece ser minutos) y de 240 a 280 unidades temporales. Puede tener relación con los dos rangos de edad detectados anteriormente.
...
Podemos observar que contamos con dos muestras no emparejadas dado que contamos con la versión del juego uno y la dos con distintos tipos de id jugador para cada una.
...
Debido a que nos encontramos con un dataframe que supera por mucho el limite de 30 que marcaria una distribución T-Student, procederemos a calcular con el metodo de distribución Normal. Además contamos con un grafico bilateral.
...
Debido a que nuestra z observada entra dentro del rango delimitado por los quartiles, podemos decir que, no podemos descartar la hipotesis nula.

### Calcular el p-value
```{r, eval=TRUE, echo=TRUE}
pnorm1 <- pnorm(zobs, lower.tail = FALSE)
```

Según el p-value podemos asegurar de que no podemos descartar la hipotesis nula con un 48% de seguridad.

### Interpretar resultados

Como conclusión creemos que, la retención entre las dos versiones es muy similar, por lo que no hay apenas diferencia entre una u otra a la hora de retener a los jugadores.

## Tiempo de Juego de los usuarios

###¿Que  tipo de inferencia es?
Tenemos la misma situación que en el ejercicio anterior, dos muestras no relacionadas separadas por versión de juego y con ids distintos.

### Escribe las hipotesis nula y alternativa
**Hº:** Version 1 <= Version 2.

**H':** Version 1 > Version 2.

### ¿Cual seria el metodo mas preciso de aproximación?

El mejor porceso para aproximarse a esta hipotesis es una distribución normal debido a la gran cantidad de datos que nos aporta el dataframe y tenemos como foco un grafico unilateral por la derecha.


### Prueba la hipotesis. Justifica que tipo de "cola" tiene.
```{r, eval=TRUE, echo=TRUE}
...
meanMG1 <- mean(MG1$`mean(SessionLenght)`)
meanMG2 <- mean(MG2$`mean(SessionLenght)`)
...
pnorm2 <- pnorm(zobs2, lower.tail = FALSE)

```
### Interpreta los resultados.

Podemos observar que rechazamos la hipotesis nula debido a que nuestra z observada se encuentra fuera del rango de nuestro quartil. Por otro lado nuestro p-vale es inferior a alpha por lo que confirmamos el descartar la hipotesis nula y nos quedamos con la alternativa.
...
```{r, eval=TRUE, echo=TRUE}
  ...
  for (user in usersData$UserID) {
    ...
    # B) Sexo del usuario ---------------------------------------------------------------------
    # Debido a que el clustering opera con VALORES NUMERICOS convertimos el character en un valor interpretable
    if (usersData[usersData$UserID == user,]$sex == "M")
      usersSex[counter] = -1   # Male
    else 
      usersSex[counter] = 1   # Female
    
    # C) tiempo de juego medio de las sesiones ---------------------------------------------
...
El identificador numérico usado en las siguientes explicaciones de cada grupo puede variar dependiendo de la ejecucion de KMeans. 
...
## ¿Que tipo de información podemos sacar de los datos? Puedes estructurar tu explicación en diferentes dimensiones del analisis: numero de jugadores,  jugabilidad, monetización, etc.

De la información obtenida podemos concluir lo siguiente:

La duración del tracking ha sido de aproximadamente 120 dias, en estos dias unos 5345 usuarios han instalado el juego durante este periodo, siendo 5245 del total los que han llegado a probarlo, pues los 1574 restantes lo desinstalaron antes de siquiera abrirlo. El tiempo promedio de juego por sesion es de 8 sesiones (8.28 para ser exactos). De la totalidad de usuarios el 40% eran mujeres, mientras que el restante 60%, hombres. Según los datos que hemos comentado anteriormente y en consecuencia a estos la tasa media de desinstalaciones es del 13.23 frente al 44.54 de instalaciones. 

Sobre la retención de usuarios, hemos podido observar como despues del primer mes estos se estabilizan alrededor de los 400, lo cual es normal en estos casos y a medida que pase el tiempo ira disminuyendo algo mas incluso. Esto es debido a que durante el lanzamiento muchisima gente se ve involucrada en el juego pero no todos continuan en el de forma constante.
En cuanto al MAUU parece es algo inferior al inicio a medida que la población del juego se estabiliza.
De todos los usuarios la gran mayoria tiene entre 6 a 10 sesiones, el valor minimo es de 1 sesión mientras que el maximo es de 17 sesiones. El tiempo medio de vida es de 11.41 dias.

El benficio total generado es de 211.248,05 unidades monetarias. El ratio de conversión es de 52.26%, muy alto dado que mas de la mitad de jugadores estan inviertiendo dinero en el juego. Según el ARPU y ARPPU hemos podido observar que la media de inversión por jugador que paga es de 77 unidades monetarias otra vez una inversión muy alta. El lifetime value es de 41.81 unidades por jugador.

Sobre hypotesis hemos confirmado que la retención de ambas versiones es muy parecida y que el tiempo medio de juego es mayor en la primera version que en la segunda.

## ¿Que problemas has encontrado?, en caso de que haya habido alguno, ¿has tenido que investigar más?.

N/A

## ¿Que recomendaciones se pueden extraer que puedan resultar utiles a los diseñadores del juego?

Según los datos extraidos nuestra recomendación es la siguiente:

Teniendo en cuenta que la población de hombres es superior y que estos son los que invierten mas en el juego, proponemos enfocar el target del juego a un publico mas masculino. No solo nos encontramos que estos invierten mas sino que presentan una situacion de juego diaria mas esporadica. Con esto queremos decir que adaptar el juego a sesiones mas cortas podria ser una buena idea.

Y no solo enfocarlo a publico masculino sino tambien a un publico mas joven ya que la media de edad del publico masculino es menor.
