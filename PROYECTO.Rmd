...
```{r, eval=TRUE, echo=TRUE}
  ...
  sesionsOfFirstMonth = sesionsPerDayWithMonth[sesionsPerDayWithMonth$Month == monthsOfAnalisis[1],]
    ...
    sesionsForthMonth = sesionsPerDayWithMonth[sesionsPerDayWithMonth$Month == monthsOfAnalisis[3],]
...
mean(usersDatesLifeTime$LifeTime)
...
usersTotal = data.frame(summarise(group_by(usersData,UserID))) #Por si hubieran user repetidos en el CSV

CR = nrow(usersMoneySpent)/nrow(usersData)

# Falta explicar resultado.
```
