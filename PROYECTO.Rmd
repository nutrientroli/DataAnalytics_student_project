...
#Retencion por dia de instalacion
  ...
  for(date in dates){
    ...
    if(check<2){
      print(date)
      usersA = data.frame(UserID=as.character(temp[temp$Action == "Install" & as.character(temp$Date) == as.character(date),]$UserID))[which()]
     
      
       result = trackingData[which(is.element(as.character(trackingData$UserID), users)),]
    
      
      
      
     # retentions[index]=RetentionOfTrackingData("DS filtrado por fecha de instalacion", c(14))
